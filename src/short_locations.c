...
char * getShortLocFiles(uint8_t locationNumber) {
    char * stringToReturn = stringRepository->loadStringFromFile(&(TextFile) {
            .entries = (void *) shortShortLocations,
            .maxLengthOfStrings = SHORT_LOC_DESCRIPTIONS_SMALL_LENGTH,
            .sizeOfIndexArray = MAX_AMOUNT_SHORTEST_LOC_SANE_DESCRIPTION,
            .amountOfEntries = AMOUNT_OF_SHORT_LOC_DESCRIPTIONS_SMALL,
    }, stringStorage, locationNumber);

    if (stringToReturn == NULL) {
        stringToReturn = stringRepository->loadStringFromFile(&(TextFile) {
                .entries = (void *) mediumShortLocations,
                .maxLengthOfStrings = SHORT_LOC_DESCRIPTIONS_MEDIUM_LENGTH,
                .sizeOfIndexArray = 1,
                .amountOfEntries = AMOUNT_OF_SHORT_LOC_DESCRIPTIONS_MEDIUM,
        }, stringStorage, locationNumber);
    }
