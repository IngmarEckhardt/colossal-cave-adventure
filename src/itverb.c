...
void ivtake(void) {
    ...
    for (item = 1; item < MAXOBJ; ++item) {
        if (objectLocation[item] == location) {
            if (anobj != 0) {
                needobj();
                return;
            }
...
void ivkill(void) {
    object1 = 0;
    if (dcheck() && dwarfFlag >= 2) { object = DWARF; }

    if (here(SNAKE)) { addobj(SNAKE); }

    if (at(DRAGON) && objectStatus[DRAGON] == 0) { addobj(DRAGON); }

    if (at(TROLL)) { addobj(TROLL); }

    if (here(BEAR) && objectStatus[BEAR] == 0) { addobj(BEAR); }
...
*/
void ivdrink(void) {
    if (liqloc(location) != WATER && (liq() != WATER || !here(BOTTLE))) {
        needobj();
    } else {
        object = WATER;
        vdrink();
...
*/
void ivquit(void) {
    if ((playerWantToQuitFlag = yes(22, 54, 54))) { normend(); }
}
...
void ivfoo(void) {
    ...
    foobar = 0;
    if (objectLocation[EGGS] == 92 || (toting(EGGS) && location == 92)) {
        rspeak(msg);
        return;
    }
