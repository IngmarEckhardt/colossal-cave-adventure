...
typedef struct {
    ...
    char description[LOC_DESCRIPTIONS_SMALL_LENGTH];
} ShortLongLocation;
...
char * getLongLocation(uint8_t locNumber) {
    ...
    for (uint8_t i = 0; i < AMOUNT_OF_LOC_DESCRIPTIONS_MEDIUM; i++) {
        if (pgm_read_byte(&mediumLongLocations[i].number) == locNumber) {
            stringToReturn = (char *) malloc(LOC_DESCRIPTIONS_MEDIUM_LENGTH);
            strcpy_P(stringToReturn, mediumLongLocations[i].description);
            return stringToReturn;
        }
    }

    for (uint8_t i = 0; i < AMOUNT_OF_LOC_DESCRIPTIONS_LONG; i++) {
        if (pgm_read_byte(&longLongLocations[i].number) == locNumber) {
            stringToReturn = (char *) malloc(LOC_DESCRIPTIONS_LONG_LENGTH);
            strcpy_P(stringToReturn, longLongLocations[i].description);
            return stringToReturn;
        }
    }
    return stringToReturn;
